...
4. **Judge detector evals** — `goal_drift` and `ungrounded_assumption` have no corpus numbers yet, because scoring them needs a live API key. Their labeled traces belong in the corpus so precision/recall can be published alongside the deterministic three.
